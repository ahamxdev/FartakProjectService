...
const IconTeacherCardArrow = lazy(() => import("@/icons/teacherCard/IconTeacherCardArrow"));
const AmozeshgahBg = lazy(() => import("@/icons/AmozeshgahBg"));

interface AmozeshgahCardProps {
  color: string;
  features: string[];
  children?: ReactNode;
}

const AmozeshgahCard = ({ color, features, children }: AmozeshgahCardProps) => {
  const router = useRouter();

  const handleButtonClick = () => {
    router.push("/courses/courseItems");
  };

  return (
    <div className="flex shrink-0 flex-col relative w-[330px] h-[426px]">
      <AmozeshgahBg style={{ color }} className="absolute inset-0" />
      <div
      ...
      <button
        ...
        className="flex bottom-0 absolute gap-2 shrink-0 py-3 cursor-pointer hover:bg-white hover:text-bluePrimary border hover:border-to-bluePrimary transition-colors duration-300 px-4 end-0 z-20 text-white bg-bluePrimary rounded-lg items-center"
      >
