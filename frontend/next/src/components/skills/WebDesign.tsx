      ...
      return (
            <>
                  ...
                  <section className="w-[90%] mx-auto md:my-10 my-5 flex flex-col items-center gap-8">
                        {
                              webCreationStep !== "web-familiar" && webCreationStep !== "web-options" ? (
                                    <p className='font-bold md:text-xl text-base text-[#000]'>{webCreationStep === "web-sub" ? "موضوع سایت" : webCreationStep === "web-type" ? "نوع سایت" : webCreationStep === "web-pages" ? "تعداد صفحات سایت" : webCreationStep === "web-time" ? "ددلاین طراحی سایت" : webCreationStep === "web-budget" ? "بودجه تخمینی سایت" : webCreationStep === "web-desc" && "توضیحات تکمیلی"} خود را انتخاب کنید</p>
                              ) : (
                                    <p className='font-bold md:text-xl text-base text-[#000]'>
                                          {webCreationStep === "web-familiar" ? "سایت های مشابه" : webCreationStep === "web-options" && "گزینه های تکمیلی (اختیاری)"}
                                    </p>
                              )
                        }
                        {
                              webCreationStep === "web-desc" || webCreationStep === "web-familiar" ? (
                                    <>
                                          {webCreationStep === "web-desc" ? (
                                                <div className='w-full flex flex-col border border-black h-[300px] gap-3 rounded-lg py-5 px-6'>
                                                      <textarea className='w-full placeholder:md:text-base placeholder:text-xs placeholder:font-bold placeholder:text-[#aaa] h-[90%]' placeholder='کمی بیشتر در مورد پروژه بنویسید و حروجی کار مد نظر خود را شرخ دهید'></textarea>
                                                      <button className='bg-[#495AFF] cursor-pointer flex justify-center items-center rounded-lg text-white  fnt-normal text-sm px-4 py-2 self-end'>مرحله بعد</button>
                                                </div>
                                          ) : webCreationStep === "web-familiar" && (
                                                <div className='w-full flex flex-col border border-black h-[300px] gap-3 rounded-lg py-5 px-6'>
                                                      <textarea className='w-full placeholder:md:text-base placeholder:text-xs placeholder:font-bold placeholder:text-[#aaa] h-[90%]' placeholder='اگر سایت مشابه ای مدنظر دارید ، لینک سایت را در اینجا بنویسید'></textarea>
                                                      <button className='bg-[#495AFF] cursor-pointer flex justify-center items-center rounded-lg text-white  fnt-normal text-sm px-4 py-2 self-end'>مرحله بعد</button>
                                                </div>
                                          )}
                                    </>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
                                          {
                                                webCreationStep === "web-sub" ? (
                                                      ...
                                                      <>
                                                            <InputCheckbox label={"سیستم مدیریت محتوا داشته باشه؟"} />
                                                            <InputCheckbox label={"سیستم مدیریت محتوا داشته باشه؟"} />
                                                            <InputCheckbox label={"سیستم مدیریت محتوا داشته باشه؟"} />
                                                            <InputCheckbox label={"سیستم مدیریت محتوا داشته باشه؟"} />
                                                      </>
